...
impl Generator {
    ...
    pub fn build<P: AsRef<Path>, Q: AsRef<Path>>(&self, md_dir: P, output_dir: Q) -> Result<()> {
        ...
        // 渲染分类页
        let categories_html = self.template_engine.render_categories(&posts, &all_categories, &all_tags)?;
        ...
        // 生成 RSS（按开关）
        let rss_enabled = self.config.data.get("features")
            .and_then(|v| v.get("rss")).and_then(|v| v.as_bool()).unwrap_or(true);
        if rss_enabled {
            self.generate_rss(&posts, output_dir)?;
        }

        // 生成 Sitemap（按开关）
        let sitemap_enabled = self.config.data.get("features")
            .and_then(|v| v.get("sitemap")).and_then(|v| v.as_bool()).unwrap_or(true);
        if sitemap_enabled {
    ...
    fn generate_tag_pages<P: AsRef<Path>>(&self, posts: &[Post], output_dir: P) -> Result<()> {
        ...
        let posts_per_page = self.config.data.get("tags")
            .and_then(|v| v.get("posts_per_page"))
            .and_then(|v| v.as_integer())
            .or_else(|| {
                self.config.data.get("homepage")
                    .and_then(|v| v.get("posts_per_page"))
                    .and_then(|v| v.as_integer())
            })
        ...
        for page in 1..=total_pages {
            let html = self.template_engine.render_tag_page(&tag_posts, &tag_name, page, posts_per_page)?;
            let file_name = if page == total_pages { "index.html".to_string() } else { format!("index{}.html", page) };
            let out_path = tag_dir.join(file_name);
            std::fs::write(&out_path, html)
                .map_err(|e| Error::Other(format!("无法写入标签分页文件 {:?}: {}", out_path, e)))?;
        }
        }
    ...
    fn generate_tag_pages_for<P: AsRef<Path>>(&self, posts: &[Post], tags: &std::collections::HashSet<String>, output_dir: P) -> Result<()> {
        let output_dir = output_dir.as_ref();
        ...
        for tag_name in tags.iter() {
            ...
            println!("标签重建: '{}' 共 {} 页 -> {}", tag_name, total_pages, rebuilt_paths.join(", "));
        }
        Ok(())
    }
    ...
    fn generate_category_pages<P: AsRef<Path>>(&self, posts: &[Post], output_dir: P) -> Result<()> {
        ...
        let posts_per_page = self.config.data.get("categories")
            .and_then(|v| v.get("posts_per_page"))
            .and_then(|v| v.as_integer())
            .or_else(|| {
                self.config.data.get("homepage")
        ...
        for category_path in category_paths.into_iter() {
            ...
            // 逐页渲染并输出（倒分页）：最大页（最新）输出为 index.html，其余为 indexN.html
            for page in 1..=total_pages {
                let html = self.template_engine.render_category_page(&category_posts, &category_path, page, posts_per_page)?;
                let file_name = if page == total_pages { "index.html".to_string() } else { format!("index{}.html", page) };
                let out_path = category_dir.join(file_name);
                std::fs::write(&out_path, html)
                    .map_err(|e| Error::Other(format!("无法写入分类分页文件 {:?}: {}", out_path, e)))?;
            }
        }
    ...
    fn generate_category_pages_for<P: AsRef<Path>>(&self, posts: &[Post], category_paths: &std::collections::HashSet<Vec<String>>, output_dir: P) -> Result<()> {
        let output_dir = output_dir.as_ref();
        ...
        for category_path in category_paths.iter() {
            ...
            println!("分类重建: '{}' 共 {} 页 -> {}", category_path.join("/"), total_pages, rebuilt_paths.join(", "));
        }
        Ok(())
    }
    
    /// 生成首页分页页面
    fn generate_index_pages<P: AsRef<Path>>(&self, posts: &[Post], all_tags: &[Value], all_categories: &Value, output_dir: P) -> Result<()> {
        let output_dir = output_dir.as_ref();
        ...
        println!("已生成首页分页页面：共{}页，路径 index{{n}}.html（首页为 index.html）", total_pages);
        
        Ok(())
    }

    /// 仅为指定页码集合生成首页分页页面（倒分页），并输出详细路径日志
    fn generate_index_pages_for<P: AsRef<Path>>(&self, posts: &[Post], all_tags: &[Value], all_categories: &Value, pages: &std::collections::HashSet<usize>, output_dir: P) -> Result<()> {
        let output_dir = output_dir.as_ref();
        ...
        let total_pages = (posts.len() + posts_per_page - 1) / posts_per_page;
        if total_pages == 0 || pages.is_empty() { return Ok(()); }

        let mut list: Vec<usize> = pages.iter().cloned().filter(|p| *p >= 1 && *p <= total_pages).collect();
        list.sort_unstable();
        ...
        println!("首页分页重建 {} 个 -> {}", rebuilt_paths.len(), rebuilt_paths.join(", "));
        Ok(())
    }
    ...
    fn generate_rss<P: AsRef<Path>>(&self, posts: &[Post], output_dir: P) -> Result<()> {
        ...
        let site_name = site.and_then(|v| v.get("name")).and_then(|v| v.as_str()).unwrap_or("RustPress Blog");
        let site_desc = site.and_then(|v| v.get("description")).and_then(|v| v.as_str()).unwrap_or("A RustPress site");
        // 优先使用自定义域名字段，其次回退 base_url
        let base_url = site
            .and_then(|v| v.get("domain")).and_then(|v| v.as_str())
            .or_else(|| site.and_then(|v| v.get("base_url")).and_then(|v| v.as_str()))
            .unwrap_or("");
    ...
    fn generate_sitemap<P: AsRef<Path>>(&self, posts: &[Post], output_dir: P) -> Result<()> {
        ...
        // 基础 URL
        let base_url = self.config.data.get("site")
            .and_then(|v| v.get("domain")).and_then(|v| v.as_str())
            .or_else(|| self.config.data.get("site").and_then(|v| v.get("base_url")).and_then(|v| v.as_str()))
            .unwrap_or("");
        let base = base_url.trim_end_matches('/');
        ...
        let mut tag_set: std::collections::HashSet<String> = std::collections::HashSet::new();
        for p in posts {
            for t in p.tags() { tag_set.insert(t.clone()); }
        }
        let tags_dir = "tags";
        for tag in tag_set.into_iter() {
            let mut tag_posts: Vec<&Post> = posts.iter().filter(|p| p.tags().contains(&tag)).collect();
            tag_posts.sort_by(|a, b| {
                let da = a.date().unwrap_or("");
                let db = b.date().unwrap_or("");
                db.cmp(da)
            ...
            let total_posts = tag_posts.len();
            let posts_per_page = self.config.data.get("tags")
                .and_then(|v| v.get("posts_per_page")).and_then(|v| v.as_integer())
                .or_else(|| self.config.data.get("homepage").and_then(|v| v.get("posts_per_page")).and_then(|v| v.as_integer()))
                .unwrap_or(8) as usize;
            let total_pages = if total_posts == 0 { 1 } else { (total_posts + posts_per_page - 1) / posts_per_page };
            for page in 1..=total_pages {
                let file_name = if page == total_pages { "index.html".to_string() } else { format!("index{}.html", page) };
        ...
        for post in posts {
            let cats = post.categories();
            if !cats.is_empty() {
                for i in 1..=cats.len() { category_paths.insert(cats[0..i].to_vec()); }
            }
        }
        let posts_per_page_cat = self.config.data.get("categories")
            .and_then(|v| v.get("posts_per_page")).and_then(|v| v.as_integer())
            .or_else(|| self.config.data.get("homepage").and_then(|v| v.get("posts_per_page")).and_then(|v| v.as_integer()))
            .unwrap_or(8) as usize;
        for path in category_paths.into_iter() {
            let mut cat_posts: Vec<&Post> = posts.iter().filter(|post| {
                let pc = post.categories();
            ...
            let total_pages = if total_posts == 0 { 1 } else { (total_posts + posts_per_page_cat - 1) / posts_per_page_cat };
            for page in 1..=total_pages {
                let file_name = if page == total_pages { "index.html".to_string() } else { format!("index{}.html", page) };
        ...
        for post in posts {
            if let Some(date) = post.date() { if date.len() >= 4 { years.insert(date[0..4].to_string()); } }
        }
        ...
        // 主要静态页面
        for static_page in [
            "tags.html", "categories.html", "archives.html",
            "about.html", "friends.html", "search.html"
        ] {
    ...
    /// 生成年份归档页面
    fn generate_year_archive_pages<P: AsRef<Path>>(&self, posts: &[Post], output_dir: P) -> Result<()> {
        let output_dir = output_dir.as_ref();
        ...
        // 按年份分组文章
        let mut year_posts: std::collections::HashMap<String, Vec<&Post>> = std::collections::HashMap::new();
        ...
        for (year, year_post_list) in year_posts {
            let year_archive_html = self.template_engine.render_year_archive(&year_post_list, &year)?;
    ...
    fn generate_year_archive_pages_for<P: AsRef<Path>>(&self, posts: &[Post], years: &std::collections::HashSet<String>, output_dir: P) -> Result<()> {
        let output_dir = output_dir.as_ref();
        ...
        for post in posts {
            if let Some(date) = post.date() {
                if date.len() >= 4 {
                    let y = &date[0..4];
                    if years.contains(y) {
                        year_posts.entry(y.to_string()).or_insert_with(Vec::new).push(post);
                    }
                }
            }
        }
    ...
    pub fn build_incremental<P: AsRef<Path>, Q: AsRef<Path>>(&self, md_dir: P, output_dir: Q) -> Result<()> {
        let md_dir = md_dir.as_ref();
        let output_dir = output_dir.as_ref();
        ...
        for &post in &changed_posts {
            ...
            if let Some(slug) = post.slug() {
                let categories = post.categories();
                let rel_dir = if categories.is_empty() { String::new() } else { categories.join("/") };
                let out_path = if rel_dir.is_empty() {
                    output_dir.join(format!("{}.html", slug))
                } else {
                    output_dir.join(format!("{}/{}.html", rel_dir, slug))
        ...
        let mut changed_categories: std::collections::HashSet<Vec<String>> = std::collections::HashSet::new();
        let mut changed_years: std::collections::HashSet<String> = std::collections::HashSet::new();

        for p in &changed_posts {
            for t in p.tags() { changed_tags.insert(t.clone()); }
            let cats = p.categories();
            if !cats.is_empty() {
                for i in 1..=cats.len() { changed_categories.insert(cats[0..i].to_vec()); }
            }
            if let Some(date) = p.date() { if date.len() >= 4 { changed_years.insert(date[0..4].to_string()); } }
        }
        ...
        for cp in &changed_posts {
            ...
            if let Some(pos) = posts_sorted.iter().position(|p| p.slug().unwrap_or("") == cs && p.categories() == cc) {
                let page = if total_pages == 0 { 1 } else { total_pages - (pos / posts_per_page) };
        ...
        if let Ok(rd) = std::fs::read_dir(output_dir) {
            for entry in rd.flatten() {
                ...
                if let Ok(name) = name_os.into_string() {
                    if let Some(rest) = name.strip_prefix("index").and_then(|s| s.strip_suffix(".html")) {
                        if let Ok(n) = rest.parse::<usize>() { existing_max_n = existing_max_n.max(n); }
                    }
                }
            }
        }
        ...
        if expected_max_n > existing_max_n {
            for n in (existing_max_n + 1)..=expected_max_n { affected_pages.insert(n); }
        }
        if !affected_pages.is_empty() {
            self.generate_index_pages_for(&posts, &all_tags, &all_categories, &affected_pages, output_dir)?;
        }
        ...
        if !changed_tags.is_empty() {
            ...
            for tag in &changed_tags {
                ...
                if !dir.exists() { need_tags_overview = true; break; }
            }
        ...
        let rss_enabled = self.config.data.get("features")
            .and_then(|v| v.get("rss")).and_then(|v| v.as_bool()).unwrap_or(true);
        if rss_enabled { self.generate_rss(&posts, output_dir)?; }

        let sitemap_enabled = self.config.data.get("features")
            .and_then(|v| v.get("sitemap")).and_then(|v| v.as_bool()).unwrap_or(true);
        if sitemap_enabled { self.generate_sitemap(&posts, output_dir)?; }
    ...
    println!("受影响标签 {} 个：{}", tags_list.len(), joined);
        ...
        if !changed_categories.is_empty() {
            let mut cat_list: Vec<String> = changed_categories.iter().map(|p| p.join("/")).collect();
            cat_list.sort();
            println!("受影响分类路径 {} 个：{}", cat_list.len(), cat_list.join(", "));
        }
        if !changed_years.is_empty() {
